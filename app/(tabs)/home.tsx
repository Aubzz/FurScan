...
const StartScreen = () => {
  ...
  // --- UPDATED: Simplified Image URL Logic for Supabase ---
  const getProfileImageUrl = () => {
    const path = user?.profileImagePath || user?.profile_image_path;
    if (!path) return null;
    
    // Supabase returns a full URL, so we just return it directly.
    return path; 
  };
      ...
      <View style={[styles.container, { paddingTop: insets.top }]}>
        ...
        >
              ...
              <TouchableOpacity activeOpacity={0.7} onPress={() => router.push('./profile')}>
                {profileImageUrl ? (
                  ...
                  <View style={[styles.profileImage, styles.profileImagePlaceholder]}>
                    <Feather name="user" size={20} color={Colors.primaryOrange} />
                  </View>
                )}
              </TouchableOpacity>
            </View>
          ...
          <View style={styles.historyContainer}>
            {isLoading ? (
              <ActivityIndicator color={Colors.primaryOrange} size="large" style={{ marginTop: 40 }} />
                  ...
                  >
                    <View style={styles.historyListItemLeft}>
                       <View style={styles.historyIconWrapper}>
                         {/* Scan Images also come as full Supabase URLs now, so this works natively! */}
                         {scan.image_uri ? (
                           <Image source={{ uri: scan.image_uri }} style={styles.historyScanImage} />
                         ) : (
                           <MaterialCommunityIcons name="paw" size={24} color={Colors.primaryOrange} />
                         )}
                       </View>
                       <View>
                    ...
                    <View style={styles.historyListItemRight}>
                      <Text style={[styles.scanSeverity, { color: scan.severity_level === 'high' ? '#D9534F' : Colors.primaryOrange }]}>
                        {scan.diagnosis_name}
                      </Text>
              ...
              <View style={styles.emptyStateWrapper}>
                <View style={styles.iconCircle}>
                  <MaterialCommunityIcons 
                      ...
                      size={70} color={Colors.primaryOrange} 
                  />
                </View>
...
const styles = StyleSheet.create({
  ...
  logoText: { fontSize: 28, fontWeight: '800', color: Colors.primaryOrange, letterSpacing: -0.5 },
  subHeaderText: { fontSize: 14, color: Colors.textSecondary, marginTop: -2 },
  profileImage: { width: 45, height: 45, borderRadius: 22.5, backgroundColor: Colors.lightOrange, borderWidth: 2, borderColor: Colors.white },
  profileImagePlaceholder: { width: 45, height: 45, borderRadius: 22.5, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: Colors.lightOrange },
  searchContainer: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  historyHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 20 },
