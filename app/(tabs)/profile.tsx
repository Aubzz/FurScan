...
const Colors = {
  ...
  primaryDark: '#E88631',   // Deeper Orange for gradient start
  primaryLight: '#FFF6EE',  // Very Light Orange
  background: '#F8F9FA',    // Light Grey Background
  card: '#FFFFFF',          // White Card
  textDark: '#2D3436',      // Dark Text
  textGrey: '#636E72',      // Grey Text
  shadow: '#000000',
  white: '#FFFFFF',
  border: '#F0F0F0',
};
...
const ProfileScreen = () => {
    ...
    if (!result.canceled && profile) {
      ...
      try {
        ...
        if (response.ok && contentType && contentType.includes("application/json")) {
          ...
          setProfile({ ...profile, profile_image_path: data.profile_image_path });
  ...
  if (error) return <View style={styles.centered}><Text>Error: {error}</Text><TouchableOpacity onPress={fetchProfile}><Text style={{ color: Colors.primary }}>Try Again</Text></TouchableOpacity></View>;
  ...
  return (
    <View style={styles.mainContainer}>
      ...
      >
        ...
        <LinearGradient
          colors={[Colors.primaryDark, Colors.primary]} // Gradient colors
          start={{ x: 0, y: 0 }} // Start at the left
          end={{ x: 1, y: 0 }}   // End at the right
          style={[styles.headerContainer, { paddingTop: insets.top + 5 }]} // Reduced from +30 to move text UP
        >
          {/* Background Stickers */}
          <View style={styles.stickerContainer}>
            <MaterialCommunityIcons name="paw" size={140} color="rgba(255,255,255,0.15)" style={[styles.sticker, { top: -40, left: -20, transform: [{ rotate: '-20deg' }] }]} />
            <MaterialCommunityIcons name="bone" size={80} color="rgba(255,255,255,0.15)" style={[styles.sticker, { bottom: 20, right: 10, transform: [{ rotate: '45deg' }] }]} />
          </View>
        ...
        <View style={styles.contentBody}>
            ...
            <View style={styles.profileHeaderCard}>
                <View style={styles.imageWrapper}>
                    ...
                    >
                        {isUploading ? <ActivityIndicator size="small" color={Colors.white} /> : <Ionicons name="camera" size={20} color={Colors.white} />}
                    </TouchableOpacity>
                </View>

                <Text style={styles.nameText}>{`${profile.first_name} ${profile.last_name}`}</Text>
                <Text style={styles.handleText}>@{profile.email.split('@')[0]}</Text>
            </View>
            ...
            <View style={styles.aboutSection}>
                <View style={styles.corgiWrapper}>
                    <Image source={require('../../assets/images/corgis.png')} style={styles.corgiImage} />
                </View>
                ...
                <View style={styles.aboutCard}>
                    ...
                    {isEditingAboutMe ? (
                        ...
                        <View style={styles.editActions}>
                            <TouchableOpacity onPress={() => setIsEditingAboutMe(false)} style={styles.cancelBtn}>
                            <Text style={styles.cancelText}>Cancel</Text>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={handleSaveAboutMe} style={styles.saveBtn}>
                        ...
                        {profile.about_me || "Tell us a bit about yourself and your furry friends!"}
                        </Text>
                    )}
                </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  handleText: { fontSize: 16, color: Colors.textGrey, marginTop: 4, fontWeight: '500' },

  sectionContainer: { marginBottom: 25, paddingHorizontal: 20 },
  sectionLabel: { fontSize: 13, fontWeight: '800', color: '#B0B0B0', marginBottom: 10, marginLeft: 5, letterSpacing: 1, textTransform: 'uppercase' },
  infoCard: {
  ...
  separator: { height: 1, backgroundColor: Colors.border, marginVertical: 12, marginLeft: 60 },
  ...
  aboutBody: { fontSize: 15, color: Colors.textDark, lineHeight: 24, fontStyle: 'italic' },
  ...
  editActions: { flexDirection: 'row', justifyContent: 'flex-end', marginTop: 15 },
  cancelBtn: { paddingVertical: 10, paddingHorizontal: 15, marginRight: 10 },
  cancelText: { color: Colors.textGrey, fontWeight: '600' },
  saveBtn: { backgroundColor: Colors.primary, paddingVertical: 10, paddingHorizontal: 24, borderRadius: 20, elevation: 2 },
