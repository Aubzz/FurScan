...
export default function ScanScreen() {
  ...
  const MODEL_API_URL = `${API_URL.replace(/:\d+$/, ":8000")}/predict`;
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        ...
        formData.append(
          "file",
          imageBlob,
          `photo.${extension === "png" ? "png" : "jpg"}`,
        );
      } else {
        const filePayload = {
          uri,
          name: `photo.${extension === "png" ? "png" : "jpg"}`,
          type: mimeType,
        };
        // @ts-ignore
      ...
      if (isAbortError) {
        Alert.alert("Scan Failed", "Model server timeout. Please try again.");
      } else {
        Alert.alert(
          "Scan Failed",
          "Cannot connect to AI model server at port 8000. Start the model backend and verify your API host/IP.",
        );
      }
    } finally {
      ...
      >
        ...
        <SafeAreaView style={styles.uiContainer}>
          ...
          <View style={styles.headerRow}>
            <TouchableOpacity
              onPress={() => router.replace("/Screens/PhotoTipsScreen" as any)}
            >
