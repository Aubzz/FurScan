  ...
  const handleLoginPress = async () => {
    ...
    try {
      ...
      try {
        ...
        console.error('Raw response starts with:', rawText.substring(0, 100));
        setError(`Server returned HTML instead of JSON. Status: ${response.status}. Check if backend is running and endpoint exists.`);
        setIsLoading(false);
        return;
      }
      ...
      setError('A network error occurred. Please check your connection and ensure backend is running.');
    } finally {
      setIsLoading(false);
    }
  };
