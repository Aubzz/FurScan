...
const MASTER_GLOSSARY: GlossaryItem[] = [
  { term: "Dermatophytosis", definition: "Clinical term for Ringworm (fungal infection)." },
  { term: "Alopecia", definition: "Technical term for hair loss." },
  { term: "Erythema", definition: "Superficial redness of the skin, sign of inflammation." },
  { term: "Zoonotic", definition: "Disease that can spread from animals to humans." }
];
...
export default function DiagnosisReportScreen() {
  ...
  const details = useMemo(() => {
    if (DISEASE_LIBRARY[currentCondition]) return DISEASE_LIBRARY[currentCondition];
    const match = Object.keys(DISEASE_LIBRARY).find(
      key => key.includes(currentCondition) || currentCondition.includes(key)
    );
    return match ? DISEASE_LIBRARY[match] : DISEASE_LIBRARY["NO SKIN DISEASE PRESENT"];
  }, [currentCondition]);
  ...
  const detectedSkinLesions = useMemo(() => {
    ...
    return lesions.length > 0 ? lesions.join(", ") : "No specific lesions detected by AI.";
  }, [allResults]);
  ...
  const triage = useMemo(() => {
    if (details.severity === 'low') {
      return { label: details.category, color: "#5CB85C", icon: "shield-checkmark" as const };
    }
    if (details.severity === 'high') {
      return { label: details.category, color: "#D9534F", icon: "alert-circle" as const };
    }
    return { label: details.category, color: "#F7924A", icon: "calendar" as const };
  }, [details]);

  const needsProfessionalHelp = details.severity === 'moderate' || details.severity === 'high';
  ...
  const findNearbyVet = () => {
    const query = details.severity === 'high' ? "Emergency Vet Clinic" : "Veterinary Clinic";
    ...
    <SafeAreaView style={styles.container}>
        ...
        <View style={styles.warningOverlay}>
          <View style={styles.warningCard}>
            ...
            <Text style={styles.warningDesc}>
              This condition is contagious. Wash hands and isolate {petName || 'your pet'}.
            </Text>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <TouchableOpacity onPress={exportPDF} disabled={loading} style={styles.iconBtn}>
            <Ionicons name={loading ? "sync-outline" : "download-outline"} size={24} color={triage.color} />
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.bodyContent}>
          ...
          <View style={styles.sectionHeader}>
            <MaterialCommunityIcons name="clipboard-text-outline" size={20} color="#666" />
          ...
          <View style={styles.careCard}>
            {(details.firstAid || []).map((item: string, i: number) => (
              <View key={i} style={styles.careRow}>
                <MaterialCommunityIcons name="medical-bag" size={16} color={triage.color} />
...
const styles = StyleSheet.create({
  ...
  resultsContainer: { backgroundColor: '#F9F9F9', padding: 15, borderRadius: 15, marginBottom: 15 },
  resultRow: { marginBottom: 12 },
  resultInfo: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 },
  ...
  careCard: { backgroundColor: '#FEF2F2', padding: 15, borderRadius: 15, gap: 12 },
  careRow: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  careText: { fontSize: 14, color: '#444', flex: 1, lineHeight: 20 },
  watchRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 },
