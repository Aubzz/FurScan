...
export default function ResultsScreen() {
  ...
  const handleProceed = () => {
    // Update context with the detected AI predictions before navigating
    const detectedIssues = results
      .filter((p) => p.label.toLowerCase() !== "healthy")
      .map((p) => p.label);

    updateAssessment({
      possibleCauses: detectedIssues,
      detectedLesions: detectedIssues, // Also saving as detected lesions
    });
    ...
    router.push({
      ...
      params: {
        imageUri,
        // aiPrediction: !isHealthy && results.length > 0 ? results[0].label : "Healthy", // Removed unused param if TellMeMore doesn't use it, but keeping logic in context is better
        petName,
        petAge,
        petBreed,
