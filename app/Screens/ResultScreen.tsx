      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.imageCard}>
          ...
          <View style={[styles.statusBadge, isHealthy ? styles.healthyBadge : styles.warningBadge]}>
            ...
            />
            <Text style={[styles.badgeText, { color: isHealthy ? "#27AE60" : "#C0392B" }]}>
              {isHealthy ? "NO DISEASES FOUND" : `${diseaseCount} ISSUE(S) DETECTED`}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.resultsWrapper}>
          {!isHealthy && results.length > 0 ? (
            <View>
              <Text style={styles.listHeader}>DETECTION DETAILS ({diseaseCount} CONDITION{diseaseCount !== 1 ? 'S' : ''})</Text>
              {results
                .filter(p => p.label.toLowerCase() !== 'healthy')
                .map((p, i) => (
                <View key={i} style={styles.progressRow}>
                  <View style={[styles.barContainer, { backgroundColor: '#FDF2F2' }]}>
                    <View style={[styles.barFill, { width: `${p.percentage}%`, backgroundColor: '#E74C3C' }]} />
                    <Text style={[styles.conditionLabel, { color: '#922B21' }]}>{p.label.toUpperCase()}</Text>
                  </View>
            ...
            <View style={styles.healthyInfoBox}>
                <Text style={styles.healthyInfoTitle}>Everything looks good!</Text>
                <Text style={styles.healthyInfoDesc}>
                    The AI analysis did not detect circular bald patches, scaling, hairloss, or redness. Continue to monitor {petName} for any changes.
                </Text>
            </View>
          )}
        </View>
...
const styles = StyleSheet.create({
  ...
  stepperContainer: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 10, marginBottom: 20 },
  dot: { width: 28, height: 28, borderRadius: 14, backgroundColor: '#E5E5E5', justifyContent: 'center', alignItems: 'center' },
  ...
  imageCard: { backgroundColor: '#FFF9F5', marginHorizontal: 20, borderRadius: 35, padding: 20, alignItems: 'center', borderWidth: 1, borderColor: '#F0E0D5' },
  scannedImage: { width: '100%', height: 240, borderRadius: 30 },
  statusBadge: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15, paddingVertical: 8, borderRadius: 20, marginTop: 15, borderWidth: 1 },
  ...
  healthyInfoBox: { backgroundColor: '#F8F9FA', padding: 20, borderRadius: 20, borderWidth: 1, borderColor: '#EEE' },
  healthyInfoTitle: { fontSize: 18, fontWeight: 'bold', color: '#333', marginBottom: 5 },
  healthyInfoDesc: { fontSize: 14, color: '#666', lineHeight: 20 },
  resultsWrapper: { paddingHorizontal: 25, marginTop: 30 },
  listHeader: { fontSize: 12, fontWeight: '800', color: '#AAA', marginBottom: 15, letterSpacing: 1 },
  progressRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 16 },
  barContainer: { height: 48, backgroundColor: '#FFF2E9', borderRadius: 24, flex: 1, marginRight: 15, overflow: 'hidden', justifyContent: 'center', paddingHorizontal: 20, position: 'relative' },
