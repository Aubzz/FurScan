...
const TIP_ITEMS = [
  { icon: 'bulb-outline', text: 'Make sure the area is well lit using natural light if possible.', type: 'ionicons' },
  { icon: 'focus-field', text: 'Keep the camera steady and in focus.', type: 'material' },
  { icon: 'magnify', text: 'Make sure the entire skin problem is visible, including some surrounding skin.', type: 'material' },
  { icon: 'hand-holding-heart', text: 'Gently part the fur if needed to clearly see the skin.', type: 'fontawesome' },
  { icon: 'image-filter-black-white', text: 'Avoid shadows, flash glare, or blurry images.', type: 'material' },
];
      ...
      <View style={styles.wrapper}>
        <View>
          ...
          <View style={styles.stepper}>
            ...
            <View style={[styles.dot, styles.activeDot]}><Text style={styles.dotText}>2</Text></View>
          ...
          <Text style={styles.subtitle}>
            Please take a clear photo of your pet's skin problem. The image will be analyzed to identify visible skin features as part of the assessment process.
          </Text>
          ...
          {TIP_ITEMS.map((tip, index) => (
            <View key={index} style={styles.tipCard}>
              <View style={styles.iconContainer}>
                {tip.type === 'ionicons' && <Ionicons name={tip.icon as any} size={24} color="#555" />}
                {tip.type === 'material' && <MaterialCommunityIcons name={tip.icon as any} size={24} color="#555" />}
                {tip.type === 'fontawesome' && <FontAwesome5 name={tip.icon as any} size={20} color="#555" />}
              </View>
...
const styles = StyleSheet.create({
  ...
  wrapper: { flex: 1, paddingHorizontal: 25, justifyContent: 'space-between', paddingBottom: 40 },
  ...
  tipCard: { backgroundColor: '#FFF5EE', flexDirection: 'row', padding: 15, borderRadius: 15, alignItems: 'center', marginBottom: 10 },
  iconContainer: { width: 40, alignItems: 'center' },
  tipText: { flex: 1, fontSize: 13, color: '#444', marginLeft: 10 },
  continueBtn: { backgroundColor: '#E89152', padding: 18, borderRadius: 20, alignItems: 'center' },
